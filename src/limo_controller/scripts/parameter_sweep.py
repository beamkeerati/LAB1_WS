...
class MPCParameterSweep(Node):
    ...
    def __init__(self):
        ...
        # Wait for user confirmation if too many experiments
        if len(self.parameter_combinations) > 50:
            self.get_logger().warn(f"WARNING: {len(self.parameter_combinations)} experiments will take a long time!")
            self.get_logger().warn("Consider reducing parameter values in the config file")
            self.get_logger().info("Starting in 10 seconds... Press Ctrl+C to cancel")
            time.sleep(10.0)
    ...
    def start_simulation_processes(self):
        ...
        try:
            ...
            self.get_logger().info("Waiting for simulation to initialize...")
            time.sleep(15.0)  # Increased wait time
            ...
            self.get_logger().info("Waiting for MPC controller to initialize...")
            time.sleep(8.0)  # Increased wait time
    ...
    def start_next_experiment(self):
        ...
        try:
            ...
            self.get_logger().info("Starting evaluator...")
            ...
            self.get_logger().info("Waiting for evaluator to start...")
            time.sleep(3.0)
            
            # Send parameter update to MPC controller
            self.get_logger().info("Sending parameter update...")
            ...
            self.get_logger().info(f"Experiment {self.current_experiment_id} running...")
    ...
    def send_parameter_update(self, parameters):
        ...
        try:
            ...
            # Wait a bit for parameters to be processed
            time.sleep(2.0)
    ...
    def finish_current_experiment(self, success=True):
        ...
        self.get_logger().info("Waiting between experiments...")
        time.sleep(5.0)
...
class MPCParameterSweep(Node):
    ...
    def create_default_sweep_config(self):
        """Create default parameter sweep configuration - UPDATED to only include yaml and switch_back paths"""
        config = {
            'parameters': {
                'target_speed': {
                    'type': 'discrete',
                ...
                'path_type': {
                    'type': 'discrete',
                    'values': ["yaml", "switch_back"]  # UPDATED: Only these two path types
                }
            },
            'evaluation_metrics': [
                'position_errors',
    ...
    def generate_path_comparison_plots(self, df, timestamp):
        """Generate plots comparing performance across different path types - UPDATED for only yaml and switch_back"""
        # Filter only successful experiments
        df_success = df[df['success'] == True].copy()
        ...
        fig.suptitle('Performance Comparison: YAML vs Switch-Back Path', fontsize=16)
    ...
    def _analyze_successful_experiments(self, df_success):
        ...
        # Best by path type (yaml vs switch_back)
    ...
    def generate_summary_report(self, df, timestamp):
        """Generate a comprehensive summary report - UPDATED for yaml and switch_back paths"""
        total_experiments = len(self.parameter_combinations)
        successful_experiments = len(df[df['success'] == True])
        failed_experiments = total_experiments - successful_experiments
...
This sweep compared two distinct path types:

### YAML Path
- **Source:** Custom path loaded from `path/path.yaml`
- **Characteristics:** Complex trajectory with multiple maneuvers and varying curvature
- **Challenge Level:** High - tests comprehensive path following capabilities

### Switch-Back Path  
- **Source:** Procedurally generated using `get_switch_back_course()`
- **Characteristics:** Challenging reversing maneuvers with tight turns
- **Challenge Level:** Very High - tests reverse driving and complex maneuvering
...
- `path_comparison_*.png` - Performance comparison between YAML and Switch-Back paths
...
3. Compare performance differences between YAML and Switch-Back paths
4. Consider running additional experiments around promising parameter regions
5. Test the recommended parameters in real-world scenarios
